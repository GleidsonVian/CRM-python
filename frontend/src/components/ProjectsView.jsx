      ...
      <div style={{ width: 560, background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 24px 80px rgba(0,0,0,0.25)' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ background: color, padding: '20px 24px 0', display: 'flex', gap: 0 }}>
          ...
          <div style={{ marginLeft: 12, flex: 1 }}>
            <div style={{ color: 'white', fontWeight: 700, fontSize: 18 }}>{name || (isNew ? 'Novo projeto' : 'Editar projeto')}</div>
            <div style={{ display: 'flex', gap: 0, marginTop: 10 }}>
              {steps.map((s, i) => (
                <button key={i} onClick={() => setStep(i)}
                  style={{ padding: '6px 16px', fontSize: 14, fontWeight: 600, cursor: 'pointer', border: 'none', fontFamily: 'inherit', background: step === i ? 'white' : 'transparent', color: step === i ? color : 'rgba(255,255,255,0.7)', borderRadius: '6px 6px 0 0', transition: 'all 0.15s' }}>
                  {s.icon} {s.label}
                </button>
              ))}
            </div>
        ...
        {step === 0 && (
          <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Nome do projeto *</label>
              <input autoFocus style={{ width: '100%', padding: '9px 12px', border: '1.5px solid #e2e8f0', borderRadius: 8, fontSize: 16, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }}
                value={name} onChange={e => setName(e.target.value)} placeholder="Nome do projeto..." />
            </div>
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Descrição</label>
              <textarea style={{ width: '100%', minHeight: 60, padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: 8, fontSize: 15, fontFamily: 'inherit', outline: 'none', resize: 'vertical', boxSizing: 'border-box' }}
                value={desc} onChange={e => setDesc(e.target.value)} placeholder="Objetivo do projeto..." />
            </div>
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Ícone</label>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {ICONS.map(ic => (
                  <button key={ic} onClick={() => setIcon(ic)} style={{ width: 36, height: 36, borderRadius: 8, border: `2px solid ${icon === ic ? color : '#e2e8f0'}`, background: icon === ic ? color + '15' : 'white', cursor: 'pointer', fontSize: 20 }}>{ic}</button>
                ))}
              </div>
            </div>
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Cor do tema</label>
        ...
        {step === 1 && (
          <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontSize: 15, color: '#64748b', marginBottom: 8 }}>Defina quem pode ver e participar deste projeto.</div>
            {Object.entries(PRIVACY_META).map(([key, meta]) => (
              <button key={key} onClick={() => setPrivacy(key)}
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 16, color: privacy === key ? color : '#0f172a' }}>{meta.label}</div>
                  <div style={{ fontSize: 14, color: '#64748b', marginTop: 2 }}>{meta.desc}</div>
                </div>
        ...
        {step === 2 && (
          <div style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Gerente do projeto</label>
              <select style={{ width: '100%', padding: '8px 10px', border: '1px solid #e2e8f0', borderRadius: 8, fontSize: 15, fontFamily: 'inherit', outline: 'none', background: 'white', boxSizing: 'border-box' }}
            ...
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Moderadores</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 130, overflowY: 'auto', border: '1px solid #e2e8f0', borderRadius: 8, padding: '6px' }}>
                {users.filter(u => u.id !== parseInt(ownerId)).map(u => (
                  <label key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', padding: '4px 6px', borderRadius: 6, background: moderatorIds.includes(u.id) ? color + '12' : 'transparent' }}>
                    ...
                    <span style={{ fontSize: 15 }}>{u.name}</span>
            ...
            <div>
              <label style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Membros da equipe</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 130, overflowY: 'auto', border: '1px solid #e2e8f0', borderRadius: 8, padding: '6px' }}>
        ...
        <div style={{ padding: '14px 24px', borderTop: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', background: '#f8fafc' }}>
          <div style={{ display: 'flex', gap: 8 }}>
            {step > 0 && <button onClick={() => setStep(step - 1)} style={{ padding: '7px 18px', borderRadius: 8, border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', fontSize: 15, fontFamily: 'inherit' }}>Voltar</button>}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={onClose} style={{ padding: '7px 18px', borderRadius: 8, border: '1px solid #e2e8f0', background: 'white', cursor: 'pointer', fontSize: 15, fontFamily: 'inherit' }}>Cancelar</button>
            {step < 2 ? (
              <button onClick={() => setStep(step + 1)} disabled={!name.trim()}
                style={{ padding: '7px 18px', borderRadius: 8, border: 'none', background: color, color: 'white', cursor: name.trim() ? 'pointer' : 'not-allowed', fontSize: 15, fontWeight: 600, fontFamily: 'inherit', opacity: name.trim() ? 1 : 0.5 }}>
                Próximo →
              </button>
            ) : (
              <button onClick={handleSave} disabled={saving || !name.trim()}
                style={{ padding: '7px 18px', borderRadius: 8, border: 'none', background: color, color: 'white', cursor: 'pointer', fontSize: 15, fontWeight: 600, fontFamily: 'inherit' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ padding: '10px 20px', background: 'white', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          <button onClick={() => setOpenProject(null)}
            style={{ background: 'none', border: '1px solid #e2e8f0', borderRadius: 7, padding: '4px 12px', cursor: 'pointer', fontSize: 14, color: '#64748b', fontFamily: 'inherit' }}>
            ← Projetos
          </button>
          <span style={{ fontSize: 22 }}>{openProject.icon}</span>
          <span style={{ fontWeight: 700, fontSize: 17, color: openProject.theme_color }}>{openProject.name}</span>
          <span style={{ fontSize: 13, color: '#94a3b8' }}>{PRIVACY_META[openProject.privacy]?.icon} {PRIVACY_META[openProject.privacy]?.label}</span>
          <button onClick={() => setEditing(openProject)} style={{ marginLeft: 'auto', background: 'none', border: '1px solid #e2e8f0', borderRadius: 7, padding: '4px 12px', cursor: 'pointer', fontSize: 14, color: '#64748b', fontFamily: 'inherit' }}>
            ✏ Editar
          </button>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: '#f8fafc' }}>
      ...
      <div style={{ padding: '14px 24px', background: 'white', borderBottom: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 18, color: '#0f172a' }}>📁 Projetos</div>
          <div style={{ fontSize: 13, color: '#64748b' }}>{projects.length} projeto{projects.length !== 1 ? 's' : ''}</div>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button onClick={fetchAll} style={{ background: 'none', border: '1px solid #e2e8f0', borderRadius: 8, padding: '5px 12px', cursor: 'pointer', fontSize: 14, color: '#64748b', fontFamily: 'inherit' }}>↻</button>
          <button onClick={() => setEditing({})}
            style={{ padding: '6px 16px', borderRadius: 8, border: 'none', background: '#6366f1', color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
            + Criar projeto
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 15, padding: '40px 0' }}>Carregando...</div>
        ) : projects.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8' }}>
            <div style={{ fontSize: 50, marginBottom: 12 }}>📁</div>
            <div style={{ fontSize: 18, fontWeight: 600, marginBottom: 8 }}>Nenhum projeto ainda</div>
            <div style={{ fontSize: 15, marginBottom: 20 }}>Crie seu primeiro projeto para organizar tarefas em equipe.</div>
            <button onClick={() => setEditing({})} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#6366f1', color: 'white', fontSize: 15, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
              + Criar projeto
            </button>
          </div>
        ) : (
                ...
                <div key={proj.id}
                  ...
                  <div style={{ padding: '16px 18px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 10 }}>
                      <div style={{ width: 44, height: 44, borderRadius: 10, background: proj.theme_color + '18', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 26, flexShrink: 0 }}>
                        {proj.icon}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 16, color: '#0f172a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{proj.name}</div>
                        <div style={{ fontSize: 13, color: '#94a3b8', marginTop: 2 }}>
                          {PRIVACY_META[proj.privacy]?.icon} {PRIVACY_META[proj.privacy]?.label}
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                        <button onClick={() => setEditing(proj)} style={{ background: 'none', border: '1px solid #e2e8f0', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', fontSize: 13, color: '#64748b', fontFamily: 'inherit' }}>✏</button>
                        <button onClick={() => handleDeleteProject(proj)} style={{ background: 'none', border: '1px solid #fecaca', borderRadius: 6, padding: '3px 8px', cursor: 'pointer', fontSize: 13, color: '#ef4444', fontFamily: 'inherit' }}>✕</button>
                      </div>
                    </div>

                    {proj.description && (
                      <div style={{ fontSize: 14, color: '#64748b', marginBottom: 10, lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                        {proj.description}
                      </div>
                    )}
