...
import SmartProcessesView from './components/SmartProcessesView';
...
function AppInner() {
  ...
  useEffect(() => {
    const handleHashChange = async () => {
      ...
      if (hash === 'smart-processes' || hash.startsWith('smart-processes/')) { setCurrentView('smart-processes'); setSelectedCard(null); return; }
  ...
  const handleDeleteStage = async (stageId) => {
    ...
    if (!stage || stage.is_terminal) return; // never delete terminal stages
    setStages(prev => prev.filter(s => s.id !== stageId));
    try { await authFetch(`${API}/stages/${stageId}`, { method: 'DELETE' }); }
    catch { setStages(prev => [...prev, stage].sort((a, b) => a.order - b.order)); }
  };
  ...
  return (
    <div className="app-wrapper">
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-section">
          <div className="sidebar-section-label">Processos</div>
          <div className={`nav-item ${currentView === 'smart-processes' ? 'active' : ''}`} onClick={() => navigate('smart-processes', 'smart-processes')}>
            ⚡ Smart Processes
          </div>
        </div>
        <div className="sidebar-section">
      ...
      <div className="main-content">
        ...
        {currentView === 'smart-processes' && <SmartProcessesView />}
